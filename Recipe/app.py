...
def remove_song_by_track_number(track_number: int) -> Response:
    ...
    try:
        ...
        return make_response(
            jsonify({'status': 'success', 'message': f'Song at track number {track_number} removed from playlist'}),
            200)
...
def move_song_to_track_number() -> Response:
    ...
    try:
        ...
        return make_response(
            jsonify({'status': 'success', 'song': f'{artist} - {title}', 'track_number': track_number}), 200)
    except Exception as e:
